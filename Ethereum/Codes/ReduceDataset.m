...
% this Matlab code reduces network's size via maximum weight subgraph approximation.
...
%for kF = 16:NFiles
    ...
    fileID = fopen([nameFolderOut '/' lisFiles(kF).name(1:(end-4)) 'XXX.txt'], 'w');
    ...
    fileID2 = fopen([nameFolderOut '/RI' lisFiles(kF).name(1:(end-4)) 'XXX.txt'], 'w');
    dataOut2 = newIndexTrimSort';
    fprintf(fileID2,'%d %d %d %-30.0f\n', dataOut2); % but truncate it so it LOOKS like an int
    dlmwrite([nameFolderOut '/INDEX' lisFiles(kF).name(1:(end-4)) 'XXX.txt'], [(1:length(lisNodes))' lisNodes], 'delimiter', ' ', 'precision', '%d')
