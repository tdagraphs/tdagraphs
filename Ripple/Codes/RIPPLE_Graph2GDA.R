...
setwd("/..../Ripple/") # working directory #

##########################
#=== Input (Edgelist) ===#
##########################

#pathEdgeListFileA = 'RIPPLE_EdgeList_A/'
#pathEdgeListFileW = 'RIPPLE_EdgeList_W/'
#pathEdgeListFileAout = 'RIPPLE_EdgeList_A_GD/'# Unweighted #
pathEdgeListFileWout = 'RIPPLE_EdgeList_W_GD/' # Weighted #
pathEdgeListINDEX = 'INDEX/'
...
for(iii in 1:NFiles){
  ...
  if(file.info(paste0(pathEdgeListFileA,lisFilesA[iii]))$size==0 && file.info(paste0(pathEdgeListFileW,lisFilesW[iii]))$size==0){
    file.create(paste0(pathEdgeListFileAout,'GD',lisFilesA[iii])) 
    file.create(paste0(pathEdgeListFileWout,'GD',lisFilesW[iii])) 
  }else{
  ...
  #--- To find the number of vertices (Unweighted) ---#
  ...
  for(kc in 1:length(vecColsA)){
    for(kf in 1:length(vecRowsA)){
      if(is.na(matDisDD_A[kc, kf])==FALSE){ # To avoid NA
        matAGD_A[vecRowsA[kc],vecColsA[kf]] <- matDisDD_A[kc, kf]
      } # End if
    } # End for kf
  } # End for cf
  ...
  for(kc in 1:length(vecColsW)){
    for(kf in 1:length(vecRowsW)){
      if(is.na(matDisDD_W[kc, kf])==FALSE){ # To avoid NA
        matAGD_W[vecRowsW[kc],vecColsW[kf]] <- matDisDD_W[kc, kf]
      } # End if
    } # End for kf
  } # End for cf
  ...
  #--- Save EdgeLists ---#
  write.table(AGD_A_EdgeList, file = paste0(pathEdgeListFileAout,'GD',lisFilesA[iii]), sep = " ", row.names = F, col.names = F) 
  write.table(AGD_W_EdgeList, file = paste0(pathEdgeListFileWout,'GD',lisFilesW[iii]), sep = " ", row.names = F, col.names = F) 
  }
}  
