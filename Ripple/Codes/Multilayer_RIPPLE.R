...
#pathPDs = 'RIPPLE_PD_A_GD/GDA' # Graph based on the Average Geodesic Distance - Unweighted
pathPDs = 'RIPPLE_PD_W_GD/GDW' # Graph based on the Average Geodesic Distance  - Weighted
...
############################################################################
#=== The strongest connections (5 tokens with the highest connectivity) ===#
############################################################################
...
for (sDay in 0:difDays) { # For each day in the time winddow
  ...
  #wasser_dist_012n = c(wasser_dist_012n, bottleneck(PDBig_ant, PDBig )) 
  PDBig_ant = PDBig 
...
# alpha=0.05 
resOutlier = AnomalyDetectionVec(data.frame(WTDA), max_anoms=0.1, period=480, direction='both', only_last=FALSE, plot=TRUE) 
...
matplot(xDays,WTDA, type = "l", col="blue", lwd=2, main = paste0("Multilayer Network"), xlab = "Week", ylab = expression(paste(W[](D[P-1],D[P]))) , add=T)
...
fileOutFoundEvents <- paste0('Results/',splitPathPD[1],'_FoundEvents.csv') # Found Events
...
fileOutFoundEventsShort <- paste0('Results/',splitPathPD[1],'_FoundEvents_Short.csv') # Found Events
