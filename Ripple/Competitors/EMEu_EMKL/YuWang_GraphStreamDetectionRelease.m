...
% ----- Open Folder name
nameFolder = 'RIPPLE_EdgeList_A/A';
%nameFolder = 'RIPPLE_EdgeList_W/W';
%nameFolder = 'RIPPLE_EdgeList_A_GD/GDA';
%nameFolder = 'RIPPLE_EdgeList_W_GD/GDW';


% ----- Open Summary
tableSummary = readtable('RIP5Y90summary.txt');
...
EuTh = quantile(EuBoot,(1-0.05/5));
%EuTh = quantile(EuBoot,(1-0.003/5));

%EuTh = max(TotalEu(3:end))*0.3;
%KLTh = max(TotalKL(3:end))*0.2;
KLBoot = datasample(TotalKL,nBootstrapSize,'Replace',true);
KLTh = quantile(KLBoot,(1-0.05/5));
%KLTh = quantile(KLBoot,(1-0.003/5));
